...
import { preloadClips } from '../utils/clipFrameLoader';
...
export function ChatArea({ user, targetUser, socket, sessionInfo, isOnline, pendingCall, clearPendingCall, dbSession, onBack, isBlocked, onUnblock, isActive, onResetSession }: ChatAreaProps) {
  ...
  const targetFpsRef = useRef<5 | 10 | 15 | 20 | 30 | 60>(10);
  ...
  useEffect(() => {
    ...
    loadLocalMessages();
  }, [sessionInfo.sessionId, sessionInfo.pin, isActive, reloadTrigger]);
  ...
  const handleApproveChatSync = async (fromId: string) => {
    try {
      ...
      showModal({ title: 'Export Failed', message: 'Could not export chat history.', iconType: 'warning' });
    }
  };
  ...
  useEffect(() => {
    ...
    const handleStealthRtpReceive = async (packet: any) => {
      ...
      // If P2P Data Channel is open, skip duplicate socket packets to prevent terrible echo/noise
    ...
    try {
      ...
      workletNode.port.onmessage = (event) => {
        if (event.data.type === 'RTP_CHUNK_READY') {
          // If direct P2P data channel is open, skip sending duplicate socket packets to save bandwidth and prevent echo
          const dc = dataChannelRef.current;
          if (dc && dc.readyState === 'open') {
            return;
          }
  ...
  const startStealthAudioDecode = async (remoteStream: MediaStream) => {
    ...
    try {
      ...
      console.error("[Stealth] Audio decoding pipeline failed:", err);
    }
  };
  ...
  const acceptCall = async () => {
    ...
    try {
      ...
      try {
        ...
        if (isVideoCall) {
          console.warn("[Stealth-Call] Failed to start video source on accept, falling back to audio-only:", err);
          showModal({
            title: 'Camera In Use / Blocked',
            message: 'Could not access your video camera. Continuing as an Audio-Only call.',
            iconType: 'warning'
